...
public class MazeSolver{
  ...
  public boolean traverse(){
    ...
    if(done){
      ...
      while(parent != null){
        maze.markPath(parent.getx(), parent.gety());
  ...
  public boolean traverse_rc(int row, int column){
    ...
    maze.tryPosition(row, column);
    if (traverse_rc(row-1, column) || traverse_rc(row, column+1) || traverse_rc(row+1, column) || traverse_rc(row, column-1)) {
      maze.markPath(row, column);
      return true;
    }
  return false;
   ...
   */
  private void push_new_pos(int x, int y, Position parent, Deque<Position> stack){
